...
Graph<T>::Graph(stack<T> stk){
    ...
    int tam = stk.size();//almacenar el tamaño del stack
    for (int i=0; i<tam;i++){ //para toda la pila de elementos
        ...
        vertexes.push_back(aux);//agregar el elemento a la lista de vertices
        stk.pop();//eliminar el primer elemento del stack
    }
    sort(vertexes.begin(), vertexes.end()); //Ordenar el vector de vertices
...
int Graph<T>::findVertex(T vertex){
    ...
    it = find(vertexes.begin(), vertexes.end(), vertex);//desde el inicio hasta el fin buscar vertex
    if (it != vertexes.end()){
        return it-vertexes.begin();
    }else{
        //cout <<"Not found"<<endl;
        return -1;
    }
