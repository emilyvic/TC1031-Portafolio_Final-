...
void DoubleLinkedList<T>::print(){
	...
	for (int i=0; i<size; i++){ //recorrer size veces
		cout <<( aux -> data) <<endl; //imprimir el dato aux
...
void DoubleLinkedList<T>::addLast(T data){
    //cout<<"data original: " <<data <<endl;
	if(isEmpty()){//si es el primer elemento que se añade
        head = new Node<T>(data, head); //crear un nodo que apunta a head, y new será el nuevo head
        tail = head;
    }else if(size == 1){//si hay un elemento en la lista
        Node<T> * aux = new Node<T>(data, tail, head);
        head->next = aux;
        tail =aux;
    }else{//si hay más de 1 elemento en la lista
        Node<T> * aux = new Node<T>(data, head, tail);
        tail->next = aux;
        tail = aux;
    }
	size ++; //incrementar el tamaño de la lista
